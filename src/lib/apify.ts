...
async function fetchLastDatasetItems(actorId: string, limit = 100) {
  ...
  const runs = await fetch(runsUrl, { cache: 'no-store' })
  if (!runs.ok) return []

  const rj = await runs.json() as any
  const dsId = rj?.data?.items?.[0]?.defaultDatasetId
  if (!dsId) return []
  ...
  const items = await fetch(itemsUrl, { cache: 'no-store' })
  if (!items.ok) return []
  const data = await items.json().catch(() => [])
  return Array.isArray(data) ? (data as ApifyListing[]) : []
}

/** Spusť actor (menší paměť), když omezení → fallback na poslední dataset. */
export async function runApifyActor(
  ...
  if (!res.ok) {
    // 402 (limit paměti) → vezmeme poslední úspěšný dataset
    if (res.status === 402) {
...
/** Namapuje položku z Apify do našeho DB tvaru. */
export function mapToUnified(item: ApifyListing, source: 'sreality'|'bezrealitky'|'ulovdomov'|'idnes') {
